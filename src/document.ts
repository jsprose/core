...
/**
 * Defines a document with autogenerated ID and no definition.
 */
...
/**
 * Defines a scoped portion of prose content with an explicit document ID and no definition.
 */
export function defineDocument(documentId: string): DocumentFinalizer<{}>;
...
export function defineDocument<const TDefinition extends DocumentDefinition>(
    ...
    if (typeof idOrDefinitionArg === 'string') {
        ...
        definition = (definitionArg ?? { uniques: {} }) as TDefinition;
    } else if (idOrDefinitionArg && typeof idOrDefinitionArg === 'object') {
        definition = idOrDefinitionArg;
        rawDocumentId = undefined;
    } else {
        definition = { uniques: {} } as TDefinition;
        rawDocumentId = undefined;
    }
    ...
    let autoUniqueCounter = 0;
    const autoUnique = (): AutoUnique => {
        let autoUniqueName: string;

        do {
            autoUniqueName = `__auto_${autoUniqueCounter++}`;
        } while (autoUniqueName in uniques);

        const unique = defineUnique({
            documentId,
            name: autoUniqueName,
            tag: autoUniqueTag as any,
        });
        (uniques as any)[autoUniqueName] = unique;
        return unique;
    };
    ...
    function finalizeDocument(
        ...
        const content = contentFunction({ uniques, autoUnique });
...
export function insertDocumentId(options: InsertDocumentIdOptions): string {
    ...
    const regex = new RegExp(`${escapedFunctionName}\\(`, 'g');
